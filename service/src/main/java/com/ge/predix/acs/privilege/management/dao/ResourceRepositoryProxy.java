...
public class ResourceRepositoryProxy implements ResourceRepository, ResourceHierarchicalRepository, InitializingBean {
    ...
    @Override
    public ResourceEntity getInheritedAttributes(final ZoneEntity zone, final String resourceIdentifier) {
        if (this.activeRepository == this.graphRepository) { // i.e. titan is enabled
            return this.graphRepository.getInheritedAttributes(zone, resourceIdentifier);
        } else {
