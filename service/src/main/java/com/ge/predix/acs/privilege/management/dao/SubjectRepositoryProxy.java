...
public class SubjectRepositoryProxy implements SubjectRepository, SubjectHierarchicalRepository, InitializingBean {
    ...
    public SubjectEntity getInheritedAttributesForScopes(final ZoneEntity zone, final String subjectIdentifier,
        ...
        if (this.activeRepository == this.graphRepository) { // i.e. titan is enabled
            return this.graphRepository.getInheritedAttributesForScopes(zone, subjectIdentifier, scopes);
        } else {
    ...
    @Override
    public SubjectEntity getInheritedAttributes(final ZoneEntity zone, final String subjectIdentifier) {
        if (this.activeRepository == this.graphRepository) { // i.e. titan is enabled
            return this.graphRepository.getInheritedAttributes(zone, subjectIdentifier);
        } else {
            return this.nonGraphRepository.getByZoneAndSubjectIdentifier(zone, subjectIdentifier);
        }
    }
